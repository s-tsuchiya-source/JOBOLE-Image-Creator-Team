...
class OpenAIImageGenerator:
    ...
    def __init__(self, model: str | None = None, quality: str | None = None):
        ...
        if not os.getenv("OPENAI_API_KEY"):
            raise ValueError(
                "OPENAI_API_KEY is required only when IMAGE_BACKEND=openai."
            )
...
class LocalWebUIImageGenerator:
    """AUTOMATIC1111 / SD-WebUI-Forge compatible local txt2img backend.

    The server runs on the user's PC. No image-generation API key or per-image
    cloud charge is required. The currently loaded WebUI checkpoint is used.
    """
    ...
    def _request_json(self, path: str, payload: dict | None = None) -> dict | list:
        ...
        try:
            ...
            raise RuntimeError(
                "Local image AI is not reachable. Start an AUTOMATIC1111/Forge-compatible "
                f"WebUI with API enabled at {self.base_url}. Original error: {exc}"
            ) from exc

    def healthcheck(self) -> dict:
        options = self._request_json("/sdapi/v1/options")
        models = self._request_json("/sdapi/v1/sd-models")
        current_model = ""
    ...
    def generate(
        ...
        *,
        ...
        result = self._request_json("/sdapi/v1/txt2img", payload)
...
def create_image_generator():
    ...
    raise ValueError(
        f"Unsupported IMAGE_BACKEND={backend!r}. Use 'local_webui' or 'openai'."
    )
